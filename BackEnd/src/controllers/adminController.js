...
const updateMembership = async (req, res) => {
  const { id } = req.params;
  const { name, gender, phoneNumber, age, address, donationAmount, payments } = req.body;

  if (!name || !gender || !phoneNumber || !age || !address) {
    return res.status(400).json({ message: 'All fields except payments and donation are required' });
  }

  try {
    const [result] = await db.query(
      'UPDATE memberships SET name = ?, gender = ?, phone_number = ?, age = ?, address = ?, donation_amount = ?, payments = ? WHERE id = ?',
      [name, gender, phoneNumber, age, address, donationAmount || null, JSON.stringify(payments), id]
    );
