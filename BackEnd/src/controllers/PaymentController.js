...
const updatePaymentStatus = async (req, res) => {
  ...
  try {
    ...
    if (Array.isArray(paymentsValue)) {
      if (paymentsValue.length === 6) {
        payments = paymentsValue.map(Boolean); // Ensure booleans
      } else {
    ...
    } else if (typeof paymentsValue === 'string' && paymentsValue.trim() !== '') {
      try {
        const parsed = JSON.parse(paymentsValue);
        if (Array.isArray(parsed) && parsed.length === 6) {
          payments = parsed.map(Boolean); // Ensure booleans
        } else {
          console.error(`Invalid payments array for member ${memberId}:`, paymentsValue);
          payments = Array(6).fill(false); // Reset to default if invalid
        }
